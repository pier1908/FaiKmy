...
vipro = LINETCR.LINE()
#vipro.login(qr=True)
vipro.login(token='Er7VmzhtZkOEVM1d8tOb.HixVftB2Kftyx0B2RLIlAW.p2wo8Dysic62BKGsRWzJtoAYMqU2v/+7NR8AmiZhTlk=')
vipro.loginResult()
...
Bots=[mid]
Creator=["u8485ef1536f69054d85f46f6ae588b6b"]
admin=["u8485ef1536f69054d85f46f6ae588b6b"]
...
#                            Name = summon(op.param2)
        ...
        if op.type == 15:
          if wait["Sambutan"] == True:
            if op.param2 in Creator:
                return
            vipro.sendText(op.param1,"What happen aya naon " + vipro.getContact(op.param2).displayName +  "\nSee You Next Time . . . (p′︵‵。) 🤗")
            d = Message(to=op.param1, from_=None, text=None, contentType=7)
            d.contentMetadata={
                                    "STKID": "7134101",
                                    "STKPKGID": "11753131",
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = vipro.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["jangan tag napa , minta desahkah...!!!"]                     
                     ret_ = random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention2"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["nge tag mele " + cName +   " mau dicipok apa mau didesah... "]
                    ret_ = random.choice(balas)
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["nah " + cName + ", cipok nih nge tag  terus!"]
                    balas1 = "inilah Artis SMULE kita . . ."
                    ret_ = random.choice(balas)
                    image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
