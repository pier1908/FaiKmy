...
vipro = LINETCR.LINE()
#vipro.login(qr=True)
vipro.login(token='EsEftLkNO7T6WYzrVO36.REewN+Nk/mQ9YCqnNxEEPG.V8Fgp+lBtucAcDqLjCHXicvAhhErdh79Cg2o7TOy/TQ=')
vipro.loginResult()
print "[Sukses Login]"
    ...
    try:
                        ...
                        if op.param1 in cctv['point']:
                            ...
                            if Name in cctv['sidermem'][op.param1]:
                                ...
                                if " " in Name:
                                    nick = Name.split(' ')
                                    if len(nick) == 0:
                                        vipro.sendText(op.param1, "kapok..    😝 "  +   Name   +   "😝  Keciduk ni ye....   ")
                                        time.sleep(0.2)
        ...
        if op.type == 15:
          if wait["Sambutan"] == True:
            if op.param2 in Creator:
                return
            vipro.sendText(op.param1,"Papay  " + vipro.getContact(op.param2).displayName +  "\nSee you next time")
            d = Message(to=op.param1, from_=None, text=None, contentType=7)
            d.contentMetadata={
                                    "STKID": "7134101",
                                    "STKPKGID": "11753131",
        ...
        if op.type == 25:
          if wait["Bot"] == True:    
			...
			vipro.sendText(msg.to,"Admin Detected~")
                ...
                vipro.sendMessage(msg)
		vipro.sendText(msg.to,"Itu Creator kami yg Jones")
		...
		if msg.from_ in admin:
		    ...
		    vipro.sendText(msg.to,"Khusus Admin")
                ...
                if wait["Sambutan"] == True:
                    if wait["lang"] == "JP":
                        vipro.sendText(msg.to,"Sambutan Di Aktifkan")
                else:
                    wait["Sambutan"] = True
                    if wait["lang"] == "JP":
                ...
                wait["Sider"] = True
                vipro.sendText(msg.to,"Siap ciduk jones alay somplak ")
                
            elif "Jones off" in msg.text:
                if msg.to in cctv['point']:
                    cctv['cyduk'][msg.to]=False
                    wait["Sider"] = False
                    vipro.sendText(msg.to, "cek jones alay somplak stop")
                else:
		...
		if wait["Simi"] == True: md+="╠➩✔️ Simisimi : On\n"
                ...
                vipro.sendText(msg.to, "check tukang ngintip")
                print "Setview"
	        ...
	        dataResult, timeSeen, contacts, userList, timelist, recheckData = [], [], [], [], [], []
                with open('dataSeen/'+msg.to+'.txt','r') as rr:
                    ...
                    if len(dataResult) > 0:
                        ...
                        subprocess.Popen("echo '' > dataSeen/"+msg.to+".txt", shell=True, stdout=subprocess.PIPE)
                        vipro.sendText(msg.to, "Auto Nyimak")                        
                    else:
                        vipro.sendText(msg.to, "Belum Ada yg keciduk")
                    print "Viewseen"
